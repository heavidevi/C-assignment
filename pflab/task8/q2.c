...
int main() {
    ...
    // 6. scalar matrix check (diagonal matrix with equal diagonal elements)
    printf("6. SCALAR MATRIX: ");
    int is_scalar = 0;
    if (is_square) {
        //  diagonal AND has equal diagonal elements
        int temp_diagonal = 1;  // Temporary diagonal check for scalar
    ...
    if (is_square && rows <= 3) {
        // We'll check if A² = 0 (simplest case)
        int is_nilpotent = 1;
